...
#define BASIC_SC                    "\"smartcard:1\": {" \
                                    "\"name\": \"label1\", \"role\": \"smartcard\", " \
                                    "\"selectable\": true, \"init_prompt\": \"Insert smartcard\", " \
                                    "\"pin_prompt\": \"PIN\"}"
...
void test_get_cert_labels(void **state)
{
    TALLOC_CTX *test_ctx = NULL;
    struct cert_auth_info *cert_list = NULL;
    ...
    ret = get_cert_labels(test_ctx, cert_list, &names);
    assert_int_equal(ret, EOK);
    assert_string_equal(names[0], SC2_LABEL);
    assert_string_equal(names[1], SC1_LABEL);
...
int main(int argc, const char *argv[])
{
    ...
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(test_get_cert_labels),
