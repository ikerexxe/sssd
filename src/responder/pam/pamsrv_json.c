...
errno_t
get_cert_labels(TALLOC_CTX *mem_ctx, struct cert_auth_info *cert_list,
                char ***_names)
{
    TALLOC_CTX *tmp_ctx = NULL;
    ...
    i = 0;
    DLIST_FOR_EACH(item, cert_list) {
        names[i] = talloc_strdup(names, item->label);
        if (names[i] == NULL) {
                           ...
                           struct pam_data *_pd)
{
    ...
    ret = get_cert_labels(tmp_ctx, cert_list, &sc_names);
    if (ret != EOK) {
        goto done;
    }
